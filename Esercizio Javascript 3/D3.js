...
for (let u = 0; u < starWarsCharacters.length; u++){
  let personaggio = starWarsCharacters[u];
...
*/
for (let v = 0; v < starWarsCharacters.length; v++){
  let character = starWarsCharacters[v];
...
while (h < starWarsCharacters.length){
  crewMass += starWarsCharacters[h].mass;
  h++;
}
...
if (crewMass < 500){
  ...
  (crewMass > 500) {
    console.log("Ship is half loader");
  }
  else if 
  (crewMass > 700) {
...
let funzionalità = Math.floor(Math.random() * starWarsCharacters.length);
console.log(starWarsCharacters[funzionalità])